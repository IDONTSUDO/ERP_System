...
const Subscription = require("../database/User/Subscriber");
...
exports.CheckBrowserForSubscription = async (req, res, next) => {
  ...
  Subscription.find({ UserAgent: userAgent, userBy: UserId }).exec(
    (err, result) => {
...
exports.DeleteDeviceByDuringExit = async (req, res, next) => {
  let userAgent = req.header("User-Agent");
  let UserId = req.auth;
  Subscription.findOneAndRemove({ UserAgent: userAgent, userBy: UserId }).exec(
    (err, result) => {
      if (err) {
