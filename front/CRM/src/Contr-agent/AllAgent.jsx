...
import { ContrAgentList, SearchContrAgent, ContrAgentDontManage } from '../Api/Http';
import { Button, Modal, Icon, Card, Table, Divider, Spin, Checkbox, Popover, Dropdown } from 'antd';
import { isAuthenticated } from '../Api/Auth';
import Error from '../Error/Error.jsx';
import { string } from 'prop-types';
import { FrownOutlined } from '@ant-design/icons';
import { UserDeleteOutlined, DownOutlined, ZoomInOutlined } from '@ant-design/icons';
...
export default class AllAgent extends Component {
	...
	render() {
		...
		let notFound = {
			emptyText: (
				<div>
					Ничего не найдено <FrownOutlined style={{ fontSize: '32px', marginRight: '5px' }} />
				</div>
			)
		};
		const columns = [
			...
			{
				title: (
					<Checkbox checked={this.state.AgentButDontManager} onChange={this.AgentButNoManagerSwitcher}>
						<UserDeleteOutlined />
					</Checkbox>
				),
				key: 'tags',
				dataIndex: 'tags',
				render: (tags) => (
					<span>{tags === 'none' ? <div className="square-red" /> : <div className="square-green" />}</span>
				)
			},
			{
				title: '',
				...
				render: (text, record) => (
					<span>
						<div style={{ padding: '5px' }}>
							<Button>
								<Link to={`/agent/${text._id}`}>Профиль</Link>
							</Button>
							<Popover content={<span />} title="Информация">
								<Button>
									<ZoomInOutlined />
								</Button>
							</Popover>
